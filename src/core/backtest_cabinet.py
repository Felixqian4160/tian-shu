...
# 数据源提供者 — 懒加载，避免未安装依赖导致导入失败
_provider_registry = {
    'default':     ('src.utils.data_provider', 'DataProvider'),
    'tushare':     ('src.utils.tushare_provider', 'TushareProvider'),
    'akshare':     ('src.utils.akshare_provider', 'AkshareProvider'),
    'mysql':       ('src.utils.mysql_provider', 'MysqlProvider'),
    'postgresql':  ('src.utils.postgres_provider', 'PostgresProvider'),
    'duckdb':      ('src.utils.duckdb_provider', 'DuckDbProvider'),
    'tdx':         ('src.utils.tdx_provider', 'TdxProvider'),
    'jarvis_duckdb': ('src.utils.jarvis_duckdb_provider', 'JarvisDuckDbProvider'),
    'tianshu':     ('src.utils.jarvis_duckdb_provider', 'JarvisDuckDbProvider'),
}
...
def _build_provider(source: str):
    ...
    if source not in _provider_registry:
        source = 'default'
    module_path, class_name = _provider_registry[source]
    cls = _lazy_import(module_path, class_name)
    if source in ('tushare',):
        from src.utils.config_loader import ConfigLoader
        return cls(token=ConfigLoader.reload().get("data_provider.tushare_token"))
    return cls()
...
class BacktestCabinet:
    ...
    def _check_provider_connectivity(self, provider, provider_source):
        try:
            ...
            if provider_source in ("jarvis_duckdb", "tianshu"):
                now = datetime.now()
                start_time = now - timedelta(days=365)  # DuckDB 数据可能不是最新的
                df = provider.fetch_minute_data(self.stock_code, start_time, now)
                if not df.empty:
                    return True, "ok"
                return False, f"DuckDB 未查询到 {self.stock_code} 的数据"
